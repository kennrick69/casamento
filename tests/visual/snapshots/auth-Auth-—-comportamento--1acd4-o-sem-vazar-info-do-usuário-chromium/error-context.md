...
- Name: auth.test.ts >> Auth — comportamento de formulários >> login com credenciais inválidas mostra erro sem vazar info do usuário
- Location: tests/e2e/auth.test.ts:79:7
...
Locator: getByText(/e-mail ou senha incorretos|verificação de segurança/i)
Expected: visible
Timeout: 12000ms
...
Call log:
  ...
  - waiting for getByText(/e-mail ou senha incorretos|verificação de segurança/i)
...
```yaml
- generic [ref=e2]: "{\"message\":\"There was a problem with the server configuration. Check the server logs for more information.\"}"
```
...
```ts
  1   | /**
  2   |  * Smoke E2E — Bloco A (Auth profissional)
  3   |  * Roda contra o servidor local (dev) ou Railway (via PLAYWRIGHT_BASE_URL).
  4   |  *
  5   |  * Cobertura mínima exigida pelo A.10:
  6   |  *   - Páginas de auth carregam corretamente
  7   |  *   - Guards de sessão redirecionam não-autenticados para /login
