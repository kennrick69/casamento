...
| Nova arquitetura de transição (HeartFlightTransition + ProtoScene como falling + FlyingScene com `casalvoando.gif`) | Integrada em produção em 2026-05-15. `src/app/page.tsx` agora renderiza `<LandingClient/>` que monta `<HeartFlightTransition trigger fallingScene={ProtoScene onUnite} flyingScene={FlyingScene src="/landing/casalvoando.gif"}/>`. `FallingScene.tsx` não está em uso (alternativa simplificada do ProtoScene; pode ser removida no futuro). **Pendentes:** (1) cor exata do céu no `COLORS.heartEnd/skyTarget` (placeholder `#FFD4B8`); (2) versão transparente do `pingpong.gif`. |
