...
| Transição do coração interna ao ProtoScene (cenário preservado) | Em produção 2026-05-15, polish completo 2026-05-17. Pixel art 16-bit (HeartFlightTransition v2 pattern): coração nasce em steps, bate 3×, power-up grow (5 paletas), SNAP instantâneo com 32 partículas. Cenário (céu/sol/nuvens) preservado. SNAP geometricamente alinhado ao centro do canvas (y=290). Queda travada no `unite()` (guard `stateRef` no setInterval). Bonecos -20% (scale no `<img>` interno), `casalvoando.gif` 204px. **Ping-pong real** via `FlyingScene.tsx` reescrito (canvas + `gifuct-js`, warm-up no mount + 4 fixes anti-halo). Botão arcade "VOCÊ É DIGNO / PRESS START" (Press Start 2P, cintilação soft + blink duro NES, glow dourado). |
